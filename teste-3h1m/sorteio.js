...
// 9 é uma meta, nunca um mínimo. A composição 3H+1M e o teto de 10
...
function criarPadroes() {
  ...
  for (let h1 = 1; h1 <= 3; h1++) for (let h2 = h1; h2 <= 3; h2++) {
    for (let h3 = h2; h3 <= 3; h3++) for (let m1 = 1; m1 <= 3; m1++) {
      const pontos = h1 + h2 + h3 + m1;
      if (pontos > REGRAS_SORTEIO.maximo) continue;
      const consumo = [0, 0, 0, 0, 0, 0];
      consumo[h1 - 1]++; consumo[h2 - 1]++; consumo[h3 - 1]++;
      consumo[m1 + 2]++;
...
function resolver(contagens, padroes, quantidade, minimo, maximo) {
  ...
  function extremo(c, quantidadeTimes, maior) {
    let soma = 0;
    for (const inicio of [0, 3]) {
      let faltam = quantidadeTimes * (inicio === 0 ? REGRAS_SORTEIO.homensPorTime : REGRAS_SORTEIO.mulheresPorTime);
  ...
  function buscar(c, faltam) {
    ...
    const menorSoma = extremo(c, faltam, false);
    const maiorSoma = extremo(c, faltam, true);
...
export function sortearTimes(atletas) {
  ...
  let quantidade = Math.min(
    Math.floor((contagens[0] + contagens[1] + contagens[2]) / REGRAS_SORTEIO.homensPorTime),
    Math.floor((contagens[3] + contagens[4] + contagens[5]) / REGRAS_SORTEIO.mulheresPorTime)
  );
  let escolhidos = null;
  while (quantidade > 0) {
